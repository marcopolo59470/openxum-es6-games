...
class Gui extends OpenXum.Gui {
    ...
    _on_click(x, y) {
        if(nb<=0 ){ // si tous les stacks sont sur la grille
            console.log("old color :"+this.old_color);
            console.log("current color :"+this._manager.get_current_color());
             if (this.old_color !== this._manager.get_current_color()) {
                 ...
                 if(coordinate[0]===0 && coordinate[1]===-1){
                     bool=true;
                     console.log("oui nord");
                     this._move = new Move(y, x, 0);
                     this.move(this._move, this._manager.get_current_color());
                 }
                 if(coordinate[0]===1 && coordinate[1]===-1) {
                     bool=true;
                     console.log("oui nord-est");
                     this._move = new Move(y, x, 1);
                     this.move(this._move, this._manager.get_current_color());
                 }
                 ...
                 if(coordinate[0]===-1 && coordinate[1]===1) {
                     bool=true;
                     console.log("oui sud-ouest");
                     this._move = new Move(y, x, 5);
                     this.move(this._move, this._manager.get_current_color());
                 }
                 if(coordinate[0]===-1 && coordinate[1]===0) {
                     bool=true;
                     console.log("oui ouest");
                     this._move = new Move(y, x, 6);
                     this.move(this._move, this._manager.get_current_color());
                 }
                 if(coordinate[0]===-1 && coordinate[1]===-1) {
                     bool=true;
                     console.log("oui nord-ouest");
                     this._move = new Move(y, x, 7);
                     this.move(this._move, this._manager.get_current_color());
                 }
                 if(coordinate[0]===0 && coordinate[1]===0) {
                     bool=true;
                     console.log("oui self");
    ...
    _draw_reserve (){
        ...
        console.log(this._engine._token_won);
        ...
        for (let a=1;a < tab.length;a+=2){
            ...
            if (tab[a]===-1) {
                ...
                switch (tab[a - 1]) {
                    case 1:
                        this._draw_piece(this._deltaY * 0.5, this._offsetX + ((a*0.32) +0.15) * this._deltaY, 'bronze');
                        break;
                    case 3:
                        this._draw_piece(this._deltaY * 0.5, this._offsetX + ((a*0.32) +0.15) * this._deltaY, 'argent');
                        break;
                    case 5:
                        this._draw_piece(this._deltaY * 0.5, this._offsetX + ((a*0.32) +0.15) * this._deltaY, 'or');
                        break;
                }
            }else{
                switch (tab[a - 1]) {
                    case 1:
                        this._draw_piece(this._offsetX + 13.4 * this._deltaX, 15.6*this._offsetX-(this._offsetX + ((a*0.32) +0.15) * this._deltaY), 'bronze');
                        break;
                    case 3:
                        this._draw_piece(this._offsetX + 13.4 * this._deltaX, 15.6*this._offsetX-(this._offsetX + ((a*0.32) +0.15) * this._deltaY), 'argent');
                        break;
                    case 5:
                        this._draw_piece(this._offsetX + 13.4 * this._deltaX, 15.6*this._offsetX-(this._offsetX + ((a*0.32) +0.15) * this._deltaY), 'or');
                        break;
                }
            }
        }
