...
let cpt_r = 15;let cpt_b = 17; //temp
...
class Gui extends OpenXum.Gui {
    ...
    if (nb > 0){
        ...
        if (coul[x][y]==='v' && this._engine.check_conflict(y,x) == 0 && this._engine.get_board_position(y,x) != -1){
            console.log(this._manager.get_current_color());
            let a = this._manager.get_current_color();
            this._draw_piece(pt[0], pt[1], a);
    ...
    _draw_token(){
        for (let i = 0; i < 13; ++i) {
            for (let j = 0; j < 13; ++j) {
                if (this._engine.get_board_position(j,i)==-1){
                    let pt = this._compute_coordinates(i, j);
                    this._draw_piece(pt[0],pt[1],'BLUE');
                }
                if (this._engine.get_board_position(j,i)==-2){
                    let pt = this._compute_coordinates(i, j);
